      ...
      <div className="flex flex-col w-full">
        ...
        <div className="flex flex-col gap-2 w-full my-10">
          ...
          <div className="w-full flex items-center justify-between gap-8 lg:flex-row sm:gap-0 lg:gap-10 flex-col">
            <TeamLineup data={watchData?.homeLineup || []} logoUrl={watchData?.gameScore.homeLogo || ""} />
            <h2 className="text-3xl lg:text-4xl text-wiz-red font-extrabold lg:ml-0 lg:mt-16 md:mt-6 md:mb-2 sm:mb-6 sm:ml-32">
              VS
            </h2>
            <TeamLineup data={watchData?.visitLineup || []} logoUrl={watchData?.gameScore.visitLogo || ""} />
            <div />
            {/* )} */}
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between w-full my-10 gap-6">
          <div className="flex flex-col gap-2 flex-1">
            ...
            {loading || !watchData ? (
              <Skeleton height={20} width="50%" className="bg-gray-200 animate-pulse " />
            ) : (
              <p className="mb-4 text-wiz-white text-sm md:text-base">{watchData.schedule.current.broadcast}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2 flex-1">
            ...
            {loading || !watchData ? (
              ...
              <p className="mb-4 text-wiz-white text-sm md:text-base">* 경기 당일 날씨만 제공됩니다.</p>
            )}
          </div>
        </div>
      </div>
